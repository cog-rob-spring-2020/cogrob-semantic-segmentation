...
def get_rot_center(alpha):
    ...
    Input: alpha is the servo angle (angle between front wheel and front (clockwise is positive, 0 is straight forward))
...
def get_distance(angle, obstacle, margin=0.5, noise_level=0):
    ...
    distance_list=sorted(distance[collide_indx]) # distance to all lidar points
    return min(distance_list[min(noise_level,len(distance_list)-1)],max_go_range) # return the minimum
